...
type HTTPContext struct {
	...
	RequestBuffer circular.Buffer
	Request       HTTPRequest

	ResponseArena arena.Arena
	ResponseIovs  []Iovec
	ResponsePos   int
...
func (rp *HTTPRequestParser) Parse(request string, r *HTTPRequest) (int, error) {
	...
	for rp.State != HTTPStateDone {
		switch rp.State {
		default:
			log.Panicf("Unknown HTTP parser state %d", rp.State)
		...
		case HTTPStateURI:
			lineEnd := strings.IndexByte(request[rp.Pos:], '\r')
			if lineEnd == -1 {
				return 0, nil
			}

			uriEnd := strings.IndexByte(request[rp.Pos:rp.Pos+lineEnd], ' ')
			if uriEnd == -1 {
				return 0, NewError("Bad Request")
			}

			queryStart := strings.IndexByte(request[rp.Pos:rp.Pos+lineEnd], '?')
			if queryStart != -1 {
...
func (w *HTTPResponse) SetCookie(name, value string, expiry time.Time) {
	...
	cookie := w.Arena.Alloc(len(name) + len(eq) + len(value) + len(path) + len(expires) + RFC822Len + len(secure))
	var n int
	n += copy(cookie[n:], name)
	n += copy(cookie[n:], eq)
...
func (w *HTTPResponse) SetHeader(header string, value string) {
	buffer := w.Arena.Alloc(len(header) + len(": ") + len(value) + len("\r\n"))
	var n int
...
func HTTPAppendResponse(wIovs *[]Iovec, w *HTTPResponse, dateIov Iovec) {
	var length int
	...
	*wIovs = append(*wIovs, IovecFromString("Content-Length: "), IovecFromBytes(lengthBuf[:n]), IovecFromString("\r\n"))
	if len(w.Bodies) != 0 {
		header := unsafe.String((*byte)(w.Bodies[0].Base), int(w.Bodies[0].Len))
		const HTMLHeader = `<!DOCTYPE html>`
		if header == HTMLHeader {
			*wIovs = append(*wIovs, IovecFromString("Content-Type: text/html; charset=\"UTF-8\"\r\n"))
		}
	}
	*wIovs = append(*wIovs, IovecFromString("\r\n"))
...
func NewHTTPContext() (*HTTPContext, error) {
	...
	return &HTTPContext{
		RequestBuffer: rb,
		ResponseIovs:  make([]Iovec, 0, 512),
		Response: HTTPResponse{
			Headers: make([]Iovec, 0, 32),
			Bodies:  make([]Iovec, 0, 128),
		},
	}, nil
}
...
func HTTPContextFromCheckedPointer(ptr unsafe.Pointer) (*HTTPContext, uintptr) {
	uptr := uintptr(ptr)
	check := uptr & 0x1
	return (*HTTPContext)(unsafe.Pointer(uptr - check)), check
}
...
func HTTPAccept(l FD, ctxPool pool.Pool[HTTPContext]) (*HTTPContext, FD, error) {
	var addr SockAddrIn
	var addrLen uint32 = uint32(unsafe.Sizeof(addr))

	c, err := Accept(l, &addr, &addrLen)
	if err != nil {
		return nil, -1, fmt.Errorf("failed to accept incoming connection: %w", err)
	}

	ctx, err := ctxPool.Get()
	if err != nil {
		Close(c)
		return nil, -1, fmt.Errorf("failed to create new HTTP context: %w", err)
	}
...
func HTTPRead(ctx *HTTPContext, conn FD) error {
	rBuf := &ctx.RequestBuffer
	n, err := Read(conn, rBuf.RemainingSlice())
	if err != nil {
		return err
	}
	rBuf.Produce(int(n))
...
func HTTPWrite(ctx *HTTPContext, conn FD) error {
	wIovs := ctx.ResponseIovs
	if len(wIovs[ctx.ResponsePos:]) == 0 {
		return nil
	}

	n, err := Writev(conn, wIovs[ctx.ResponsePos:])
	if err != nil {
		return err
	}
...
func StartAccept(ctx *HTTPContext) AcceptCtx {
	ctx.Request.Address = ctx.ClientAddress
	return AcceptCtx{ctx}
}

func (ctx AcceptCtx) Read() (*HTTPResponse, *HTTPRequest, bool) {
	n, err := ctx.RequestParser.Parse(ctx.RequestBuffer.UnconsumedString(), &ctx.Request)
	if err != nil {
		log.Println("[ERROR] Failed to parse HTTP request:", err.Error())
	}
	if n == 0 {
		return nil, nil, false
	}

	ctx.RequestBuffer.Consume(n)
	ctx.Response.Reset()
	return &ctx.Response, &ctx.Request, true
}

func (ctx AcceptCtx) Done() {
	ctx.Request.Reset()
	HTTPAppendResponse(&ctx.ResponseIovs, &ctx.Response, IovecFromBytes(unsafe.Slice(&ctx.DateBuf[0], len(ctx.DateBuf))))
}
