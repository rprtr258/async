...
func PageCourse(w *HTTPResponse, r *HTTPRequest) error {
...
func PageCourseLesson(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
...
func PageCourseCreateEdit(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	if id == "" {
		...
		SetInt(r.Form, "ID", len(user.Courses)-1)
	} else {
	...
	case "Add lesson":
		...
		SetInt(r.Form, "LessonIndex", len(course.Lessons)-1)
		return LessonAddPageHandler(w, r, lesson)
	case "Continue":
		li, err := GetValidIndex(r.Form.Get("LessonIndex"), course.Lessons)
		if err != nil {
	...
	case "Add test":
		...
		SetInt(r.Form, "StepIndex", len(lesson.Steps)-1)
...
func HandlerCourseDelete(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
