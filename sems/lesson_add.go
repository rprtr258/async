...
func LessonTestFillFromRequest(vs url.Values, test *StepTest) error {
	...
	for i := 0; i < len(questions); i++ {
		...
		answers := vs[unsafe.String(unsafe.SliceData(answerKey), len("Answer")+n)]
		...
		correctAnswers := vs[unsafe.String(unsafe.SliceData(correctAnswerKey), len("CorrectAnswer")+n)]
...
func LessonProgrammingFillFromRequest(vs url.Values, task *StepProgramming) error {
	...
	for i := 0; i < len(CheckKeys); i++ {
		...
		inputs := vs[CheckKeys[i][CheckKeyInput]]
		outputs := vs[CheckKeys[i][CheckKeyOutput]]
