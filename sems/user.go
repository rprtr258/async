...
func PageUser(w *HTTPResponse, r *HTTPRequest) error {
...
func PageUserCreate(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
...
func PageUserEdit(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
...
func PageUserSignin(w *HTTPResponse, r *HTTPRequest) error {
	w.AppendString(`<!DOCTYPE html>`)
...
func HandlerUserCreate(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	if err := UserNameValid(firstName); err != nil {
		return WritePage(w, r, PageUserCreate, err)
	}
	...
	if err != nil {
		return WritePage(w, r, PageUserCreate, BadRequest("provided email is not valid"))
	}
	...
	if !StringLengthInRange(password, MinPasswordLen, MaxPasswordLen) {
		return WritePage(w, r, PageUserCreate, BadRequest("password length must be between %d and %d characters long", MinPasswordLen, MaxPasswordLen))
	}
	if password != repeatPassword {
		return WritePage(w, r, PageUserCreate, BadRequest("passwords do not match each other"))
	}

	if GetUserByEmail(email) != nil {
		return WritePage(w, r, PageUserCreate, Conflict("user with this email already exists"))
	}
...
func HandlerUserEdit(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	firstName := r.Form.Get("FirstName")
	if err := UserNameValid(firstName); err != nil {
		return WritePage(w, r, PageUserEdit, err)
	}
	...
	address, err := mail.ParseAddress(r.Form.Get("Email"))
	if err != nil {
		return WritePage(w, r, PageUserEdit, BadRequest("provided email is not valid"))
	}
	email := address.Address
	...
	repeatPassword := r.Form.Get("RepeatPassword")
	if !StringLengthInRange(password, MinPasswordLen, MaxPasswordLen) {
		return WritePage(w, r, PageUserEdit, BadRequest("password length must be between %d and %d characters long", MinPasswordLen, MaxPasswordLen))
	}
	if password != repeatPassword {
		return WritePage(w, r, PageUserEdit, BadRequest("passwords do not match each other"))
	}

	user := GetUserByEmail(email)
	if (user != nil) && (user.ID != userID) {
		return WritePage(w, r, PageUserEdit, Conflict("user with this email already exists"))
	}
...
func HandlerUserSignin(w *HTTPResponse, r *HTTPRequest) error {
	if err := r.ParseForm(); err != nil {
		return ClientError(err)
	}
	...
	user := GetUserByEmail(email)
	if user == nil {
		return WritePage(w, r, PageUserSignin, NotFound("user with this email does not exist"))
	}

	password := r.Form.Get("Password")
	if user.Password != password {
		return WritePage(w, r, PageUserSignin, Conflict("provided password is incorrect"))
	}
...
func HandlerUserSignout(w *HTTPResponse, r *HTTPRequest) error {
