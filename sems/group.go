...
func PageGroup(w *HTTPResponse, r *HTTPRequest) error {
...
func PageGroupCreate(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	DisplayStudentsSelect(w, r.Form["StudentID"])
	w.AppendString(`</label>`)
	w.AppendString(`<br><br>`)
...
func PageGroupEdit(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
...
func HandlerGroupCreate(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	if !StringLengthInRange(name, MinGroupNameLen, MaxGroupNameLen) {
		return WritePage(w, r, PageGroupCreate, BadRequest("group name length must be between %d and %d characters long", MinGroupNameLen, MaxGroupNameLen))
	}

	sids := r.Form["StudentID"]
...
func HandlerGroupEdit(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	name := r.Form.Get("Name")
	if !StringLengthInRange(name, MinGroupNameLen, MaxGroupNameLen) {
		return WritePage(w, r, PageGroupEdit, BadRequest("group name length must be between %d and %d characters long", MinGroupNameLen, MaxGroupNameLen))
	}
