...
type (
	...
	SubmittedTest struct {
		...
		/* TODO: I don't like this. Replace with 'pointer|1'. */
		Draft bool
	}
...
func GetSubmittedStepScore(step any, submittedStep any) (int, int) {
	var nsteps int
...
func PageSubmission(w *HTTPResponse, r *HTTPRequest) error {
	...
	for k, v := range r.Form {
		...
		if strings.HasPrefix(k, "Command") {
			/* NOTE: after command is executed, function must return. */
...
func SubmissionNewTestVerifyRequest(vs url.Values, submittedTest *SubmittedTest) error {
	...
	for i := 0; i < len(test.Questions); i++ {
		...
		selectedAnswers := vs[unsafe.String(unsafe.SliceData(selectedAnswerKey), len("SelectedAnswer")+n)]
...
func PageSubmissionNew(w *HTTPResponse, r *HTTPRequest) error {
	session, err := GetSessionFromRequest(r)
	if err != nil {
		return UnauthorizedError
	}
	...
	if submissionIndex == "" {
		...
		submission.SubmittedSteps = make([]any, len(lesson.Steps))

		lesson.Submissions = append(lesson.Submissions, submission)
		SetInt(r.Form, "SubmissionIndex", len(lesson.Submissions)-1)
	} else {
